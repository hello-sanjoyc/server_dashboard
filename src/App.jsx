...
const apiBaseUrl = configuredApiBaseUrl.replace(/\/+$/, '').replace(/\/api$/, '');
...
function UsageBar({ value, threshold }) {
  ...
  return (
    <div className="usage-bar" aria-label={`${normalized}% used`}>
      <div className={danger ? 'danger' : ''} style={{ width: `${normalized}%` }} />
    </div>
  );
}
...
function Dashboard({ user, onLogout }) {
  ...
  const thresholds = metrics?.thresholds || { cpuPercent: 50, ramPercent: 50 };
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(loadMetrics, metrics.refreshSeconds * 1000);
        ...
        <>
          <section className="metrics-grid">
            <MetricCard
              ...
              tone={metrics.system.cpu.usagePercent > thresholds.cpuPercent ? 'hot' : ''}
            />
            <MetricCard
              ...
              tone={metrics.system.memory.usagePercent > thresholds.ramPercent ? 'hot' : ''}
            />
            <MetricCard
          ...
          <section className="process-section">
            <div className="section-heading">
              <div>
                <h2>PM2 Processes</h2>
              <p>
                Updated {formatDate(metrics.generatedAt)}. Auto-refresh every{' '}
                {metrics.refreshSeconds} seconds.
              </p>
              {metrics.pm2Error && <p className="warning">PM2 unavailable: {metrics.pm2Error}</p>}
            ...
            <span>{metrics.pm2.length} process(es)</span>
          </div>
              ...
              <table>
                ...
                <tbody>
                  {metrics.pm2.map((process) => {
                    const overCpu = process.cpuPercent > thresholds.cpuPercent;
                    const overRam = process.ramPercent > thresholds.ramPercent;
                    return (
                      <tr key={`${process.id}-${process.name}`}>
                        <td>
                          <strong>{process.name}</strong>
                          <small>#{process.id} / {process.namespace}</small>
                        </td>
                        <td>
                        ...
                        <td>
                          <div className="usage-cell">
                            <span className={overCpu ? 'danger-text' : ''}>
                              {process.cpuPercent}%
                            </span>
                            <UsageBar value={process.cpuPercent} threshold={thresholds.cpuPercent} />
                          </div>
                        </td>
                        <td>
                          <div className="usage-cell">
                            <span className={overRam ? 'danger-text' : ''}>
                              {process.ramPercent}% / {process.ramHuman}
                            </span>
                            <UsageBar value={process.ramPercent} threshold={thresholds.ramPercent} />
                          </div>
                        </td>
                        <td>{ageFromTimestamp(process.uptime)}</td>
