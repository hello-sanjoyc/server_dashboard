...
const config = {
  ...
  alertIntervalSeconds: Number(process.env.ALERT_CHECK_INTERVAL_SECONDS || 60),
...
async function getSystemMetrics() {
  ...
  const networkStats = networkResult.status === 'fulfilled' ? networkResult.value : [];
  const interfaces = Array.isArray(networkStats) ? networkStats : [networkStats];
  ...
  return {
    ...
    warnings: [
      loadResult.status === 'rejected' ? `CPU metrics unavailable: ${loadResult.reason.message}` : null,
...
async function getPm2ProcessMetrics(totalMemoryBytes = os.totalmem()) {
  ...
  return list.map((processInfo) => {
    ...
    const ramPercent = totalMemoryBytes > 0 ? (memoryBytes / totalMemoryBytes) * 100 : 0;
...
async function sendAlertEmail(processes) {
  ...
  if (!transporter) {
    console.warn('Alert skipped: SMTP_HOST, EMAIL_FROM, and EMAIL_TO must be configured.');
    return;
  }
